...
namespace MovieStoreMvc.Controllers
{
    ...
    public class MovieController : Controller
    {
        ...
        [HttpPost]
        public IActionResult AddtoCart(Movie model)
        {
            model.GenreList = _genService.List().Select(a => new SelectListItem { Text = a.GenreName, Value = a.Id.ToString() });
            if (!ModelState.IsValid)
                return View(model);
            var result = _movieService.AddToCart(model);
            if (result)
            {
                TempData["msg"] = "Added to Cart successfull";
                return RedirectToAction(nameof(Add));
            }
            else
            {
        ...
        {
            var data = this._movieService.CartList();
            return View(data);
        }
